...
def parse_args(args=None):
    ...
    parser.add_argument('-dis', '--distance_type', default="cosine",
                        type=str, help='type of inside and outside distance ["sine", "cosine", "angle"]')
    parser.add_argument('-p', '--psi_axis_distance', default=0.9,
                        type=float, help="weight of axis distance")
    parser.add_argument('-delta', '--delta_none', default=0.5,
                        type=float, help="weight of axis for none type of intersection between sector-cones")
    ...
    parser.add_argument('--geo', default='scone', type=str, choices=['noname', 'scone'],
                        help='list of model names')
    parser.add_argument('-cenr', '--center_reg', default=0.02, type=float,
                        help='center_reg to balances the dist in and dist out')
    parser.add_argument('-projm', '--proj_mode', default="(1600,2)", type=str,
                        help='(hidden_dim,num_layer) for relational projection')
    parser.add_argument('-projn', '--proj_net', default="rtrans_mlp", type=str,
                        help='select network in ["rtrans_mlp", "rtrans", "mlp"] for relational projection')
    parser.add_argument('-att', '--att_mode', default="stack", type=str,
                        help='attention mode in ["stack", "dot_prod", "scaled_dot_prod"] for entanglement gate')
    parser.add_argument('-logic', '--logic_mode', default="geometry", type=str,
                        help='logic mode using geometry or attention')
    parser.add_argument('-conj', '--conj_mode', default="all", type=str,
                        help='conjunction mode to calculate intersection of two geometric shapes ["all", "partial", "complete", "none"]')
...
def main(args):
    ...
    if args.geo in ['scone']:
        tmp_str = "g-{}-hdim_layer-({}.{})-dim-{}".format(
    ...
    shutil.copy("model.py", args.save_path)
    shutil.copy("main.py", args.save_path)
    print(f"copying model model.py and main.py to {args.save_path}")
    ...
    model = KGReasoning(
        ...
        psi_axis_distance=args.psi_axis_distance,
        distance_type=args.distance_type,
        proj_net=args.proj_net,
        conj_mode=args.conj_mode,
        att_mode=args.att_mode,
        delta_none=args.delta_none,
    )
    ...
    if args.geo == 'scone':
        logging.info(
            f"{args.geo} mode (proj_mode, inside_dis_lambda) = ({args.proj_mode}, {args.center_reg})")
    ...
    logging.info(f"logic mode = {args.logic_mode}")
    logging.info(f"conjunction mode = {args.conj_mode}")
    logging.info('psi = %.2f' % args.psi_axis_distance)
    logging.info('distance type = %s' % args.distance_type)
    logging.info('projection network = %s' % args.proj_net)
    logging.info('attention mode = %s' % args.att_mode)
    logging.info('delta none intersection type = %.1f' % args.delta_none)
